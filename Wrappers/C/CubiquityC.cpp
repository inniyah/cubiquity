...
// Volume functions
////////////////////////////////////////////////////////////////////////////////
CUBIQUITYC_API int32_t cuNewColouredCubesVolume(int32_t lowerX, int32_t lowerY, int32_t lowerZ, int32_t upperX, int32_t upperY, int32_t upperZ, uint32_t baseNodeSize, const char* folderName, uint32_t* result)
{
	OPEN_C_INTERFACE

	ColouredCubesVolume* volume = new ColouredCubesVolume(Region(lowerX, lowerY, lowerZ, upperX, upperY, upperZ), baseNodeSize, folderName);
...
CUBIQUITYC_API int32_t cuNewColouredCubesVolumeFromVolDat(const char* volDatToImport, uint32_t baseNodeSize, const char* folderName, uint32_t* result)
{
	OPEN_C_INTERFACE

	ColouredCubesVolume* volume = importVolDat<ColouredCubesVolume>(volDatToImport, baseNodeSize, folderName);
	volume->markAsModified(volume->getEnclosingRegion(), UpdatePriorities::Immediate); //Immediate update just while we do unity experiments.
