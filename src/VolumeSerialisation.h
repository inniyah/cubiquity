	...
	for (int z = 0; z < volume->mVolData->getWidth(); z++)
	{
		for (int y = 0; y < 32; y++)  //HACK - hardcoded value!!!
		{
			for (int x = 0; x < volume->mVolData->getDepth(); x++)
			{
				...
				size_t elementsRead = fread(&value, sizeof(VolumeType::VoxelType), 1, inputFile);
...
void saveData(const char* filename, VolumeType* volume)
{
	...
	for (int z = 0; z < volume->mVolData->getWidth(); z++)
	{
		for (int y = 0; y < volume->mVolData->getHeight(); y++)
		{
			for (int x = 0; x < volume->mVolData->getDepth(); x++)
			{
				VolumeType::VoxelType value = mVolData->getVoxelAt(x,y,z);
				fwrite(&value, sizeof(VolumeType::VoxelType), 1, outputFile);
			}
		}
	}
