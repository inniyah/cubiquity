...
namespace Cubiquity
{
	...
	void SmoothTerrainVolumeEditor::edit(const Vector3F& centre, float radius, uint32_t materialToUse, EditAction editAction, float timeElapsedInSeconds, float amount, float smoothBias)
	{
		...
		if(editAction == EditActions::Smooth)
		{
			...
			::PolyVox::LowPassFilter< ::PolyVox::SimpleVolume<MultiMaterial>, ::PolyVox::RawVolume<MultiMaterial>, ::PolyVox::Vector<4, float> > lowPassFilter(getPolyVoxVolumeFrom(mSmoothTerrainVolume), region, mSmoothingVolume, region, 3);
